    ...
    if (!cin) {
        ...
        if (my_armours.empty()) {
            ...
            if (!cin) { // user didn't input a number
                ...
                if (my_armours.at(decision - 1)->isInUse())
                    cout << "You're already wearing it!\n";
                else if (my_armours.at(decision - 1)->getExperienceLevel() > this->getLevel())
                    cout << "You haven't reached the required Level yet to wear this Armour!\n";
                else {
...
void Enemy::displayStats(){
    ...
    cout << "\n Damage: " << damage << " \n Defence: " <<defence << " \n Dodge Probability: " <<dodge_rate*100 << "%\n";
}
void Player::displayStats() {
    ...
    cout << "\n Money: " << money << "$ \n";
    cout << "\n Defensive Skills: \n";
    if (my_defensiveSkills.empty()) {
        cout << " No defensive skills acquired.\n";
    }
    else {
        for (const auto& skill : my_defensiveSkills) {
            cout << " - " << skill->getName() << " (Effectiveness: " << skill->getEffectiveness() * 100 << "%, Duration: " << skill->getDuration() << " turns)\n";
        }
    }
}
void Player::buy(Weapon* weapon) {
    int decision;
...
void Player::buy(Spell* spell) {
    this->money = money - spell->getPrice();
    cout << endl << spell->getName();
    this->acquire(spell);
    cout << " acquired!\n\n";
}
void Player::acquire(Weapon* weapon) {
    my_weapons.push_back(weapon);
}
void Player::acquire(Armor* armour) {
    my_armours.push_back(armour);
}
void Player::acquire(Potion* potion) {
    my_potions.push_back(potion);
}
void Player::acquire(Spell* spell) {
    my_spells.push_back(spell);
}
double Player::getMoney() const {
    return money;
}
bool Player::has_something_to_sell() {
    ...
    if (!cin) { // user didn't input an unsigned integer number
        ...
        if (my_weapons.empty()) cout << "\n You don't have any Weapons for sale! \n\n";
            ...
            if (!cin) { // user didn't input a number
                ...
                vector<Weapon*>::iterator weapon_iterator;
                weapon_iterator = my_weapons.begin();
                weapon_iterator = weapon_iterator + decision2 - 1;
                delete my_weapons.at(decision2 - 1);
                my_weapons.erase(weapon_iterator);
        ...
        if (my_armours.empty()) cout << "\n You don't have any Armours for sale! \n\n";
            ...
            if (!cin) { // user didn't input a number
                ...
                if (cashmoney > money) money = cashmoney;
                vector<Armor*>::iterator armor_iterator;
                armor_iterator = my_armours.begin();
                armor_iterator = armor_iterator + decision2 - 1;
                delete my_armours.at(decision2 - 1);
                my_armours.erase(armor_iterator);
                cout << " has been succesfuly sold! \n\n";
                cout << " Your new balance is: " << money << "$\n\n";
            }
        }
        ...
        if (my_potions.empty()) cout << "\n You don't have any Potions for sale! \n\n";
            ...
            if (!cin) { // user didn't input a number
                ...
                if (cashmoney > money) money = cashmoney;
                vector<Potion*>::iterator potion_iterator;
                potion_iterator = my_potions.begin();
                potion_iterator = potion_iterator + decision2 - 1;
                delete my_potions.at(decision2 - 1);
                my_potions.erase(potion_iterator);
                cout << " has been succesfuly sold! \n\n";
                cout << " Your new balance is: " << money << "$\n\n";
            }
        }
        ...
        if (my_spells.empty()) cout << "\n You don't have any Potions for sale! \n\n";
            ...
            if (!cin) { // user didn't input a number
                ...
                if (cashmoney > money) money = cashmoney;
                vector<Spell*>::iterator spell_iterator;
                spell_iterator = my_spells.begin();
                spell_iterator = spell_iterator + decision2 - 1;
                delete my_spells.at(decision2 - 1);
                my_spells.erase(spell_iterator);
                cout << " has been succesfuly sold! \n\n";
                cout << " Your new balance is: " << money << "$\n\n";
            }
        }
...
Spell* Player::castSpell(int how_many_spells) {
    int decision;
    Spell* temp = NULL;
    do {
        int k = 1;
    ...
    for (int i = 0; i < my_spells.size(); i++) {
        if ((my_spells.at(i)->getExperienceLevel() <= this->getLevel()) && (my_spells.at(i)->getMagicalEnergy() <= this->magicPower)) {
            if (decision == lcount) {
                ...
                for (int j = 0; j < my_potions.size(); j++) {
                    if ((my_potions.at(j)->isInUse()) && (my_potions.at(j)->getAttributeImprovement() == "Dexterity")) { //And to an active dexterity potion
                        finaldamage *= my_potions.at(j)->getIncrement();
                        vector<Potion*>::iterator potion_iterator;
                        potion_iterator = my_potions.begin();
                        potion_iterator = potion_iterator + j;
                        delete my_potions.at(j);
                        my_potions.erase(potion_iterator);
                    }
                }
...
int Enemy::defendFrom(Spell* evilSpell) {
    int spellPower = 0;
    int dodging = int(dodge_rate * 100);
...
bool Enemy::defendFrom(int attack_power) { //Defending from an attack
    int defensive = int(defence);
    int dodging = int(dodge_rate * 100);
    if (under_a_spell.empty() == false && under_a_spell.front()->getType() == "Lightning Spell") {
...
void Enemy::release_me_from_my_spell() {
    if (!under_a_spell.empty()) {
...
void Enemy::cleanup_spell_space() {
    if (!under_a_spell.empty()) {
        delete under_a_spell.front();
        under_a_spell.pop_back();
    }
...
int Player::attack() {
    ...
    for (int i = 0; i < my_potions.size(); i++) {
        if (my_potions.at(i)->isInUse() && my_potions.at(i)->getAttributeImprovement() == "Strength") { //Using the active potion
            ...
            vector<Potion*>::iterator potion_iterator; //and then removing it
            potion_iterator = my_potions.begin();
            potion_iterator += i;
    ...
    for (int j = 0; j < my_potions.size(); j++) { //Removing a potion from their system in case they used dexterity potion, but didn't cast a spell
        if ((my_potions.at(j)->isInUse()) && (my_potions.at(j)->getAttributeImprovement() == "Dexterity")) {
            vector<Potion*>::iterator potion_iterator;
            potion_iterator = my_potions.begin();
            potion_iterator = potion_iterator + j;
            delete my_potions.at(j);
...
void Player::setExperience(double xp) {
