...
namespace Color {
    ...
    // Original color modifiers
    Modifier red(BG_RED);
    Modifier black(FG_BLACK);
    Modifier yellow(BG_YELLOW);
    Modifier water(BG_BLUE);
    Modifier grass(BG_GREEN);
    Modifier def(FG_DEFAULT);
    Modifier def2(BG_DEFAULT);

    // New color modifiers for quadrants
    Modifier quadrant1(BG_QUADRANT1);
    Modifier quadrant2(BG_QUADRANT2);
    Modifier quadrant3(BG_QUADRANT3);
    Modifier quadrant4(BG_QUADRANT4);
}
void Map::displayMap() {
    cout << endl;
    cout << " ";
        ...
        for (j = 0; j <= 2 * gridColumns; j++) {
            if (i % 2 == 0) {
                ...
                if (j % 2 == 0) {
                    ...
                    if (i / 2 < gridRows / 2 && j / 2 < gridColumns / 2) {
                        cout << Color::quadrant1 << " " << Color::def2;
                    }
                    else if (i / 2 < gridRows / 2 && j / 2 >= gridColumns / 2) {
                        cout << Color::quadrant2 << " " << Color::def2;
                    }
                    else if (i / 2 >= gridRows / 2 && j / 2 < gridColumns / 2) {
                        cout << Color::quadrant3 << " " << Color::def2;
                    }
                    else {
                        cout << Color::quadrant4 << " " << Color::def2;
                    }
                }
                else {
                    cout << Color::def2 << "---";
                ...
                if (j % 2 == 0)
                    ...
                    case 1:
                        if (board[(i - 1) / 2][(j - 1) / 2]->isHeroesPresent())
                            cout << Color::yellow << Color::black << "^_^" << Color::def << Color::def2;
                        else
                            cout << Color::def2 << "   ";
                        break;
                    case 2:
                        ...
                        break;
                    case 3:
                        if ((positionX == (i - 1) / 2) && (positionY == (j - 1) / 2))
                            cout << Color::yellow << Color::black << "^_^" << Color::def << Color::def2;
                        else
                            cout << Color::red << " M " << Color::def2;
                        break;
                    }
                }
            }
    ...
    cout << endl << endl;
    cout << "You: " << Color::yellow << Color::black << "^_^" << Color::def << Color::def2 << "     ";
    cout << "Market: " << Color::red << " M " << Color::def2 << "     ";
    cout << "Black is Land(walkable): ";
    cout << "Lake (inaccesible): " << Color::water << "   " << Color::def2;
    cout << endl;
    cout << "First Quadrant (Forest & Town): " << Color::quadrant1 << "   " << Color::def2
        << "   Second Quadrant (Caves & Mountains): " << Color::quadrant2 << "   " << Color::def2
        << "   Third Quadrant (Desert): " << Color::quadrant3 << "   " << Color::def2
        << "   Fourth Quadrant (Enemy Stronghold): " << Color::quadrant4 << "   " << Color::def2
        << endl;
...
Map::Map() {
    ...
    this->positionY = 9;
    //Dragons
    monsters[24]= new DarkBeasts("Yolen", 9, 125, 36, 800, 0.5);
    monsters[23]= new DarkBeasts("Threnody", 8, 90, 32, 650, 0.4);
    monsters[22]= new DarkBeasts("Catacendre",7, 85,  23, 420, 0.3);
    monsters[21]= new DarkBeasts("Sleepless",5, 75,  16, 60, 0.2);
    //Exoskeletons
    monsters[20]= new Undead("Ruin",9, 120, 33, 900, 0.3);
    monsters[19]=new  Undead("Inquisitor", 8, 100, 31, 700, 0.24);
    monsters[18]= new Undead("Marsh",7, 90, 20, 550, 0.2);
    monsters[17]= new Undead("Quellion",6, 80, 18, 200, 0.18);
    monsters[16]= new Undead("Roshar",5, 70, 13, 70, 0.15);
    monsters[15]=new  Undead("Scadrial",4, 60, 12, 38, 0.12);
    monsters[14]= new Undead("Harms",3, 40, 8, 20, 0.1);
    monsters[13]= new Undead("Seran",2, 25, 5, 8, 0.08);
    monsters[12]= new Undead("Gin",1, 10,  2, 2, 0.05);
    //Spirits
    monsters[11]= new Demons("Adonalsium", 10, 140, 38, 1000, 0.72);
    monsters[10]= new Demons("TenSoon", 9, 120, 34, 750, 0.65);
    monsters[9]= new Demons("MeLaan", 8, 100, 30, 600, 0.6);
    monsters[8]= new Demons("Waxillium", 7, 95, 21, 360, 0.55);
    monsters[7]= new Demons("Renarin", 6, 85, 18, 140, 0.50);
    monsters[6]= new Demons("Haddek", 5, 75, 14, 52, 0.45);
    monsters[5]= new Demons("VarSel", 4, 65, 13, 30, 0.35);
    monsters[4]= new Demons("Ladrian", 3, 50, 9, 16, 0.33);
    monsters[3]= new Demons("KanPaar", 3, 40, 8, 14, 0.3);
    monsters[2]= new Demons("RaShek", 2, 30, 7, 6, 0.28);
    monsters[1]= new Demons("OreSeur", 2, 25, 6, 5, 0.26);
    monsters[0]= new Demons("Kandra", 1, 10, 2, 1, 0.2);
    //Weapons
    ...
    //Market Squares
    board[0][10] =new  market(weapons, armours, potions, spells, 3, "Raja Bazaar");
    board[6][16] = new market(weapons, armours, potions, spells, 3, "Anarkali Bazaar");
    board[9][9] = new market(weapons, armours, potions, spells, 3, "Kohinoor Market");
    board[15][3] = new market(weapons,armours, potions, spells, 3, "Looneywala Market");
    board[17][18] = new market(weapons, armours, potions, spells,  3, " Hathyaar Bazaar");
}
void Map::setHeroSize(int parhero = 1) {
    this->heroes = parhero;
}
void Map::place_on_the_board(Player* hero, int X = 18, int Y = 9) {
    board[X][Y]->place(hero);
    board[X][Y]->setHeroesPresent(true);
    positionX = X;
    positionY = Y;
}
void Map::playerOptions() {
    if (positionX - 1 >= 0 && board[positionX - 1][positionY]->getType() != 2) {
        cout << " 1. Move Up \t\t\t\t\t";
    }
    else cout << "Moving Up is Inaccessible \t\t\t\t";
    if (positionX + 1 < gridColumns && board[positionX + 1][positionY]->getType() != 2) {
        cout << " 2. Move Down \n\n";
    }
    else cout << "Moving Down is Inaccessible \n\n";
    if (positionY - 1 >= 0 && board[positionX][positionY - 1]->getType() != 2) {
        cout << " 3. Move Left \t\t\t\t\t";
    }
    else cout << "Moving Left is Inaccessible \t\t\t\t";
    ...
    cout << " 7. Quit and Save Game \n\n";
}
bool Map::check_option(int option){
    if (option==5||option==6||option==7) return true;
    else if (option == 1) {
...
void Map::move(int movement) {
    ...
    else if (movement == 4) { //Move Right
        for (int i = 0; i < heroes; i++) board[positionX][positionY + 1]->place(board[positionX][positionY]->exit());
        board[positionX][positionY + 1]->setHeroesPresent(true);
        board[positionX][positionY]->setHeroesPresent(false);
        positionY = positionY + 1;
    }
    ofstream saveFile("savegame.dat", ios::trunc);
    if (saveFile.is_open()) {
        saveFile <<positionX << " " <<positionY << endl;
        saveFile.close();
    }
}
bool Map::inside_a_market(){
    if (board[positionX][positionY]->getType()==3) return true;
    return false;
}
void Square::place(Player* hero){
...
void Map::battle(){
    ...
    for (int i=0; i<monster_count;i++){
        ...
        monster_team.add(new Enemy(*monsters[pick_a_monster]));
    }
    ...
    while (!winner) {
        ...
        for (int i = 0; i < heroes_here.size(); i++) { // Heroes' turn
            ...
            GoBack:
                ...
                if (course_of_action == 3) {
                    // Display defensive skills
                    cout << "Choose a defensive skill to use:\n";
                    for (int j = 0; j < heroes_here.at(i)->my_defensiveSkills.size(); j++) {
                        cout << j + 1 << ". " << heroes_here.at(i)->my_defensiveSkills[j]->getName() << endl;
                    }
                    int skillChoice;
                    cin >> skillChoice;
                    heroes_here.at(i)->useDefensiveSkill(skillChoice - 1);
                }
                    ...
                    // Calculate damage considering defensive skills
				...
				else if (course_of_action == 4) {
					heroes_here.at(i)->checkInventory();
				}
				else goto GoBack; // Invalid option, go back to the menu
            }
        }
        ...
        for (int i = 0; i < heroes_here.size(); i++) {
            if (heroes_here.at(i)->getHealthPower() != 0) { // If this hero is not unconscious
                heroes_here.at(i)->recover(); // Rejuvenate's a part of the health & magic power
            }
        }
        for (int i = 0; i < teamMonster.size(); i++) teamMonster.at(i)->recover(); // Rejuvenate a part of the monsters health
        round++;
    }
endgame:
...
void Square::shopping_time() {
    ...
    for (int i = 0; i < heroes_here.size(); i++) {
        ...
        if (decide == 1) {
            ...
            if (heroes_here.at(i)->has_something_to_sell()) {
...
void market::display_stock(int option, Player* customer) { //Display the available items of the selected kind
...
void Square::display_stock(int j, Player* heroas) {
    cout<< "I'm a virtual function that isn't used\n";
}
void Map::restock() { //RESTOCKING THE MARKETS
